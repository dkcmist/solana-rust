...
async function addPool(poolAccounts: any) {
  ...
  const ammId = new anchor.web3.PublicKey(poolAccounts.ammId);
  const ammAuthority = new anchor.web3.PublicKey(poolAccounts.ammAuthority);
  const ammOpenOrders = new anchor.web3.PublicKey(poolAccounts.ammOpenOrders);
  const ammTargetOrders = new anchor.web3.PublicKey(
    poolAccounts.ammTargetOrders
  );
  const poolCoinTokenAccount = new anchor.web3.PublicKey(
    poolAccounts.poolCoinTokenAccount
  );
  const poolPcTokenAccount = new anchor.web3.PublicKey(
    poolAccounts.poolPcTokenAccount
  );

  const serumMarket = new anchor.web3.PublicKey(poolAccounts.serumMarket);
  const serumBids = new anchor.web3.PublicKey(poolAccounts.serumBids);
  const serumAsks = new anchor.web3.PublicKey(poolAccounts.serumAsks);
  const serumEventQueue = new anchor.web3.PublicKey(
    poolAccounts.serumEventQueue
  );
  const serumCoinVaultAccount = new anchor.web3.PublicKey(
    poolAccounts.serumCoinVaultAccount
  );
  const serumPcVaultAccount = new anchor.web3.PublicKey(
    poolAccounts.serumPcVaultAccount
  );
  const serumVaultSigner = new anchor.web3.PublicKey(
    poolAccounts.serumVaultSigner
  );
...
async function main() {
  // await addPool(USDTUSDC);
  // await addPool(USDCRAY);
  // await addPool(TESTTEST);
  for (let i = 0; i < LIQUIDITY_POOLS.length; i++) {
    console.log(i, "---", LIQUIDITY_POOLS[i].name);
    await addPool(LIQUIDITY_POOLS[i]);
  }
}
